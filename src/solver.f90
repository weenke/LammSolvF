...
contains
    ...
    function sawp_solver(c0, t0, n_steps, omega, s, D, m, b, N, dt, &
        ...
        real(dp) :: s_G, alpha_dt, alpha_1
        real(dp) :: initial_mass, current_mass_sum, inv_rad_w_last, denom
        integer  :: step, i, lapack_info

        ! --- B and J diagonals at t0 and t0+dt ---
        real(dp) :: B0_sub(N-1), B0_diag(N), B0_sup(N-1)
        real(dp) :: J0_sub(N-1), J0_diag(N), J0_sup(N-1)
        real(dp) :: B1_sub(N-1), B1_diag(N), B1_sup(N-1)
        real(dp) :: J1_sub(N-1), J1_diag(N), J1_sup(N-1)

        ! --- lhs = (B0+B1) - dt*J1,  rhs = (B0+B1) + dt*J0 ---
        real(dp) :: lhs_sub(N-1), lhs_diag(N), lhs_sup(N-1)
        real(dp) :: rhs_sub(N-1), rhs_diag(N), rhs_sup(N-1)
        real(dp) :: lhs_du2(N-2)
        integer  :: ipiv(N)

        real(dp) :: b_vec(N), c_new(N), c_interp(N)

        ! --- Interpolation weights ---
        integer  :: idx(N)
        real(dp) :: w_L(N), w_R(N)

        real(dp) :: radial_weights(N)
        real(dp) :: nodes_t0(N), nodes_tpdt(N)
        ...
        do i = 1, N
            ...
            if (nodes_t0(i) < nodes_tpdt(1)) then
                w_L(i) = 1.0_dp ;  w_R(i) = 0.0_dp ;  idx(i) = 2
            else if (nodes_t0(i) > nodes_tpdt(N)) then
                w_L(i) = 0.0_dp ;  w_R(i) = 1.0_dp ;  idx(i) = N
            end if
        end do
        ...
        inv_rad_w_last = 1.0_dp / radial_weights(N)

        ! --- 7. Initialise output ---
        do step = 1, n_steps + 1
            times_out(step) = t0 + real(step - 1, dp) * dt
        end do
        coeffs_out(:, 1) = c0
        ...
        do step = 1, n_steps
            ...
            associate(c_next => coeffs_out(:, step + 1))
                ...
                c_next(1)     = c_new(1)
                c_next(2)     = 0.5_dp * (c_next(1) + c_next(3))
